...
def seed_default_fixtures() -> Dict[str, List[Dict[str, Any]]]:
    ensure_directories()
    sources = list_sources()
    documents = list_documents()
    if not sources:
        gfr = create_source(
            title="General Financial Rules (source pending verification)",
    ...
    if not documents:
        for source in sources:
            if source["source_type"] == "GFR":
                ...
                break
    return {"sources": list_sources(), "documents": list_documents()}
